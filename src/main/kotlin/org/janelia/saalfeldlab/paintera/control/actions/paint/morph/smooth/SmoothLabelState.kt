...
import org.janelia.saalfeldlab.paintera.control.actions.paint.morph.smooth.SmoothLabel.SmoothScope
...
internal open class SmoothLabelState<D, T>(delegate: SmoothLabelModel = SmoothLabelModel.default()) :
	...
	@Synchronized
	fun getSmoothedCellImage(labelsToSmooth: LongArray, blocksWithLabels: Set<Interval>, cellDimensions: IntArray? = null): SmoothedCellImage {

		return reuseSmoothedImage(labelsToSmooth, blocksWithLabels, cellDimensions) ?: setSmoothedImage(labelsToSmooth, blocksWithLabels, cellDimensions)
	}

	@Synchronized
	private fun setSmoothedImage(labels: LongArray, blocksWithLabels: Set<Interval>, cellDimensions: IntArray? = null): SmoothedCellImage {
	...
	@Synchronized
	private fun reuseSmoothedImage(labelsToSmooth: LongArray, blocksWithLabel: Set<Interval>, cellDimensions: IntArray? = null): SmoothedCellImage? {
	...
	suspend fun smoothMask(preview: Boolean = false, update: UpdateSignal = UpdateSignal.Full, cellDimensions: IntArray? = null): Set<RealInterval> {
		...
		val intervalsWithLabel = if (preview) {
			...
			blocksWithLabel.filter { labelBlock ->
				viewerIntervals.any { viewerInterval -> viewerInterval.intersect(labelBlock).isNotEmpty() }
			}.toSet()
		} else
			blocksWithLabel
		...
		* instead of always doing the max. This should otherwise be safe to do though.*/
		val intervalsToProcess = intervalsWithLabel.mapTo(mutableSetOf()) { it.extendBy(*smoothedCellImage.kernelSizePadding) }
