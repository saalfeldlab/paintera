...
data class SmoothedCellImage(
	...
	fun invalidatedImageOrNull(
		...
		val canInvalidate = canInvalidate(kernelSize, labelsToSmooth, intervalsToSmooth, infillStrategy, replacementLabel, threshold)
		if (canInvalidate) {
	...
	companion object {
		fun createSmoothedCellImage(
			...
			val erodedCellImage = ErodedCellImage.createErodedCellImage(initialLabels, labelsToSmooth, kernelSize, resolution, infillStrategy, replacementLabel, intervalsToSmooth, cellDimensions)
			val dilatedCellImage = DilatedCellImage.createDilatedCellImage(initialLabels, labelsToSmooth, kernelSize, resolution, infillStrategy, replacementLabel, intervalsToSmooth, cellDimensions)
			...
			val (gaussianSmoothingMask) = MorphOperations.paddedCellGaussianSmoothing(cellDimensions, kernelSizeInPixels, labelsMask, sigma)
			...
			val smoothedImg = DiskCachedCellImgFactory(
				...
				val initialLabelsCursor = extendedInitialLabels.interval(smoothedCell).cursor()
				...
				while (smoothedLabelsCursor.hasNext()) {
					...
					val wasLabel by lazy { labelsMaskCursor.get().get() == 1.0 }

					var smoothFillLabel = Label.TRANSPARENT
					if (gaussian < threshold) {
						if (shrink && wasLabel) {
							smoothFillLabel = when (infill) {
						...
						if (expand && !wasLabel) {
							smoothFillLabel = dilatedLabelsCursor.get().get()
						}
