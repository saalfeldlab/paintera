...
import org.janelia.saalfeldlab.paintera.control.actions.paint.morph.erode.ErodeLabel.mainTaskLoop
...
internal class ErodeLabelUI(val model: ErodeLabelModel) : VBox(10.0) {

	companion object {
		fun getDialog(model: ErodeLabelModel, titleText: String = "Shrink Label", onApply: () -> Unit): Dialog<Boolean> {
			...
			return Dialog<Boolean>().apply {
				...
				title = titleText
				val erodeLabelUI = ErodeLabelUI(model)
				dialogPane.content = erodeLabelUI
				...
				dialogPane.lookupButton(ButtonType.APPLY).also { applyButton ->
					val disableBinding = model.canApply.and(erodeLabelUI.kernelIsChanging.not()).map { !it }
						...
						mainTaskLoop?.invokeOnCompletion { cause ->
							when (cause) {
								...
								is CancellationException -> {
									LOG.info { "User cancelled shrink label" }
									InvokeOnJavaFXApplicationThread {
...
fun main() {
	InvokeOnJavaFXApplicationThread {
		...
		val model = object : ErodeLabelModel by ErodeLabelModel.default() {

			override fun getLevelResolution(scaleLevel: Int) = doubleArrayOf(1.0, 1.0, 1.0)
		}

		val dialog = ErodeLabelUI.getDialog(model, "Shrink Label") { println("Done!") }.apply {
			...
			(dialogPane.lookupButton(reloadButton) as? Button)?.addEventFilter(ActionEvent.ACTION) {
				dialogPane.content = ErodeLabelUI(model)
				it.consume()
			}
		}
		...
		InvokeOnJavaFXApplicationThread {
			delay(200)
			val statuses = listOf(Status.Empty, ErodeStatus.Eroding, Status.Applying, Status.Done)
