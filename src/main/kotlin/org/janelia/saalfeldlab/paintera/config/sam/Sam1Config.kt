...
import org.janelia.saalfeldlab.paintera.config.sam.Sam1Config.Companion.DEFAULT_COMPRESS_ENCODING
import org.janelia.saalfeldlab.paintera.config.sam.Sam1Config.Companion.DEFAULT_IMAGE_ENCODING
import org.janelia.saalfeldlab.paintera.config.sam.SamModelConfig.Companion.SamModelData
import org.janelia.saalfeldlab.paintera.config.sam.SamTritonConfig
...
class Sam1Config : SamModelConfig<Sam1Config>(
	DEFAULT_SERVICE_URL,
	...
	override fun equals(other: Any?): Boolean {
        if (other !is Sam1Config) return false
	...
	companion object {
		...
		internal data class Sam1Data(
			...
			constructor(config: Sam1Config)  : this(
				SamModelData(config),
...
class Sam1ConfigNode(val config: Sam1Config) : SamModelConfigNode(config) {
...
class Sam1Adapter : SamModelConfigAdapter<Sam1Config>() {

	override fun getTargetClass() = Sam1Config::class.java
	override fun newInstance() = Sam1Config()

	override fun serialize(src: Sam1Config, typeOfSrc: Type, context: JsonSerializationContext): JsonElement {
	...
	override fun deserialize(json: JsonElement?, typeOfT: Type, context: JsonDeserializationContext): Sam1Config {
