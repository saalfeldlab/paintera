...
class KeyAndMouseConfig {
    ...
    fun getConfigFor(source: SourceState<*, *>): KeyAndMouseBindings {
        ...
        sourceSpecificBindings.keyCombinations.keys.forEach {
            it.takeUnless { propertiesBindings.keyCombinations.keys.contains(it) }
                ?.let { propertiesBindings.keyCombinations.addCombination(sourceSpecificBindings.keyCombinations[it]!!) }
        }
        sourceSpecificBindings.mouseCombinations.keys.forEach {
            it.takeUnless { propertiesBindings.mouseCombinations.keys.contains(it) }
                ?.let { propertiesBindings.mouseCombinations.addCombination(sourceSpecificBindings.mouseCombinations[it]!!) }
        }
