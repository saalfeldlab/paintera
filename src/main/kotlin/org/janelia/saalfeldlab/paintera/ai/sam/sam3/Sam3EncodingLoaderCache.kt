...
class Sam3EncodingLoaderCache : ImageEncodingLoaderCache<Sam3TrackerEncoderResult>() {
    ...
    override val cache: LoaderCache<RenderUnitState, Deferred<Sam3TrackerEncoderResult>> = SoftRefLoaderCache()

    override val embeddingRequester: ImageEmbeddingRequester<Sam3TrackerEncoderResult> by LazyForeignValue({
//        paintera.properties.segmentAnythingConfig.run { "$serviceUrl?responseTimeout=${responseTimeout}" }
    }) {
        Sam3EmbeddingRequester()
    }

    override tailrec suspend fun loader(key: RenderUnitState): Sam3TrackerEncoderResult {
    ...
    override val healthCheck
        get() = embeddingRequester.healthCheck()
    ...
    override fun request(viewer: ViewerPanelFX, globalToViewerTransform: AffineTransform3D): Deferred<Sam3TrackerEncoderResult> {
