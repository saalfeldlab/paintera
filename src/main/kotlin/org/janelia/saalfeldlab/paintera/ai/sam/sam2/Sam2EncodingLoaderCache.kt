...
class Sam2EncodingLoaderCache : ImageEncodingLoaderCache<Sam2EncoderResult>(), AutoCloseable {
    ...
    override val cache: LoaderCache<RenderUnitState, Deferred<Sam2EncoderResult>> = SoftRefLoaderCache()

    override val embeddingRequester: ImageEmbeddingRequester<Sam2EncoderResult> by LazyForeignValue({
//        paintera.properties.segmentAnythingConfig.run { "$serviceUrl?responseTimeout=${responseTimeout}" }
    }) {
        Sam2EmbeddingRequester()
    }

    override tailrec suspend fun loader(key: RenderUnitState): Sam2EncoderResult {
    ...
    override fun request(viewer: ViewerPanelFX, globalToViewerTransform: AffineTransform3D): Deferred<Sam2EncoderResult> {
