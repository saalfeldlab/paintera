        ...
        return json.getN5MetaAndTransform(context)
            ...
            ?.let { (meta, transform) ->
                ...
                ConnectomicsLabelState(
                    ...
                    .also { s ->
                        with(GsonExtensions) {
                            json.getJsonObject("converter")?.getLongProperty("seed")?.let { s.converter().seedProperty().set(it) }
                        }
                    }
                    .also { s ->
                        with(GsonExtensions) {
                            json.getJsonObject("converter")?.getJsonObject("userSpecifiedColors")?.let { s.converter().setCustomColorsFromJson(it) }
                        }
                    }
                    .also { s ->
                        with(GsonExtensions) {
                    ...
                    .also { s ->
                        with(GsonExtensions) {
                            s.applyActions(
                                json.getProperty("assignment")?.getProperty("data")?.getJsonArray("actions"),
                                context
                            )
                        }
                    }
                    ...
                    .also { s ->
                        with(GsonExtensions) {
                            json.getProperty("lockedSegments")?.let { context.deserialize<LongArray>(it, LongArray::class.java) }
                                ?.forEach { s.lockedSegments.lock(it) }
                        }
                    }
            } ?: run {
    ...
    companion object {
        ...
        private fun ConnectomicsLabelState<*, *>.applyActions(
            ...
            json
                ?.map { it.asJsonObject }
                ?.map { entry ->
                    val type: AssignmentAction.Type =
                        context.deserialize(entry.get(FragmentSegmentAssignmentOnlyLocalSerializer.TYPE_KEY), AssignmentAction.Type::class.java)
