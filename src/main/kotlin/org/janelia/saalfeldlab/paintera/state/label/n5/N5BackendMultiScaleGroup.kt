	...
	companion object {
		...
		fun createBackend(
			...
			if (isLabelMultisetType)
				return N5BackendMultiScaleGroup<LabelMultisetType, VolatileLabelMultisetType>(container, dataset, labelBlockLookup, resolution, offset, queue, priority, name, projectDirectory, propagationExecutorService)
			return when (dataType) {
					DataType.INT64 -> N5BackendMultiScaleGroup<LongType, VolatileLongType>(container, dataset, labelBlockLookup, resolution, offset, queue, priority, name, projectDirectory, propagationExecutorService)
					DataType.UINT32 -> N5BackendMultiScaleGroup<UnsignedIntType, VolatileUnsignedIntType>(container, dataset, labelBlockLookup, resolution, offset, queue, priority, name, projectDirectory, propagationExecutorService)
					DataType.UINT64 -> N5BackendMultiScaleGroup<UnsignedLongType, VolatileUnsignedLongType>(container, dataset, labelBlockLookup, resolution, offset, queue, priority, name, projectDirectory, propagationExecutorService)
