...
class BorderPaneWithStatusBars(private val paintera: PainteraMainWindow) {
    ...
    private val currentSourceName = MenuItem(null)
        .also {
            it.textProperty().bind(center.sourceInfo().currentState().let { Bindings.createStringBinding(Callable { it.value?.nameProperty()?.value }, it) })
        }
    ...
    private val cycleForward = MenuItem("Cycle _Forward")
        .also {
            it.acceleratorProperty().bind(namedKeyCombinations[PainteraMainWindow.BindingKeys.CYCLE_CURRENT_SOURCE_FORWARD]!!.primaryCombinationProperty())
        }
    ...
    private val cycleBackward = MenuItem("Cycle _Backward")
        .also {
            it.acceleratorProperty().bind(namedKeyCombinations[PainteraMainWindow.BindingKeys.CYCLE_CURRENT_SOURCE_BACKWARD]!!.primaryCombinationProperty())
        }
    ...
    private val toggleVisibility = MenuItem("Toggle _Visibility")
        .also {
            it.acceleratorProperty().bind(namedKeyCombinations[PainteraMainWindow.BindingKeys.TOGGLE_CURRENT_SOURCE_VISIBILITY]!!.primaryCombinationProperty())
        }
