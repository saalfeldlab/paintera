...
public class GenericBackendDialogN5 implements Closeable {
	...
	private final StringBinding datasetPath = Bindings.createStringBinding(
			() -> Optional.ofNullable(this.activeN5Node.get()).map(N5TreeNode::getPath).orElse(null), activeN5Node);
	...
	private final BooleanBinding isDatasetValid = Bindings.createBooleanBinding(
			() -> activeN5Node.get() != null && !Objects.equals(getDatasetPath(), EMPTY_STRING), activeN5Node);
	...
	private MenuButton createDatasetDropdownMenu(String datasetPromptText) {
		...
		final var datasetDropDownDisable = Bindings.createBooleanBinding(() -> this.datasetChoices.isEmpty() || this.discoveryIsActive.get(),
				this.datasetChoices, this.discoveryIsActive);
