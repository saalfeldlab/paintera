...
public interface DataSource< D, T > extends Source< T >
{
	...
	public static < T > DataSource< T, T > fromSource( final Source< T > source )
	{
		return new DataSource< T, T >()
		{
			...
			@Override
			public RandomAccessibleInterval< T > getDataSource( final int t, final int level )
			{
				return getSource( t, level );
			}

			@Override
			public RealRandomAccessible< T > getInterpolatedDataSource( final int t, final int level, final Interpolation method )
			{
				return getInterpolatedSource( t, level, method );
			}
