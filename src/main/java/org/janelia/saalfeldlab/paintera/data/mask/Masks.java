...
public class Masks {
	...
	public static <I extends IntegerType<I> & NativeType<I>, V extends AbstractVolatileRealType<I, V>> MaskedSource<I,
		...
		for (int level = 0; level < blockSizes.length; ++level) {
			/* a sliced source is a view; getGrid returns the underlying 3D block grid (see DataSource.getGrid) */
			final int[] blockSize = new int[source.getGrid(level).numDimensions()];
			source.getGrid(level).cellDimensions(blockSize);
	...
	public static Supplier<String> canvasTmpDirDirectorySupplier(final String projectCacheDir) {

		return new TmpDirectoryCreator(Paths.get(projectCacheDir, "canvases"), "canvas-");
	}
