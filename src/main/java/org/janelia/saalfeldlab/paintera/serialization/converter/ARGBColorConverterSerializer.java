...
public class ARGBColorConverterSerializer< T extends RealType< T > > implements JsonSerializer< ARGBColorConverter< T > >, JsonDeserializer< ARGBColorConverter< T > >
{
	...
	public ARGBColorConverter< T > deserialize( final JsonElement json, final Type type, final JsonDeserializationContext context ) throws JsonParseException
	{
		try
		{
			...
			final ARGBColorConverter< T > converter = ( ARGBColorConverter< T > ) Class.forName( type.getTypeName() ).getConstructor( double.class, double.class ).newInstance( 0.0, 255.0 );
			final JsonObject map = json.getAsJsonObject();
			Optional.ofNullable( map.get( ALPHA_KEY ) ).map( JsonElement::getAsDouble ).ifPresent( converter.alphaProperty()::set );
			Optional.ofNullable( map.get( COLOR_KEY ) ).map( JsonElement::getAsString ).map( Colors::toARGBType ).ifPresent( converter.colorProperty()::set );
	...
	@Override
	public JsonElement serialize( final ARGBColorConverter< T > src, final Type type, final JsonSerializationContext cpntext )
	{
