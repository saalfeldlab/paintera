...
public class IntersectingSourceStateSerializer implements JsonSerializer< IntersectingSourceState >
{
	...
	public static class Factory implements StatefulSerializer.Serializer< IntersectingSourceState, IntersectingSourceStateSerializer >
	{
	...
	@Override
	public JsonObject serialize( final IntersectingSourceState state, final Type type, final JsonSerializationContext context )
	{
		final JsonObject map = new JsonObject();
		map.addProperty( NAME_KEY, state.nameProperty().get() );
		map.add( SourceStateSerialization.DEPENDS_ON_KEY, context.serialize( Arrays.stream( state.dependsOn() ).mapToInt( stateToIndex ).toArray() ) );
