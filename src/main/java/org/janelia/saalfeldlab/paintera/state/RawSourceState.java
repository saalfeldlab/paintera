...
public class RawSourceState< D, T extends RealType< T > > extends MinimalSourceState< D, T, DataSource< D, T >, ARGBColorConverter< T > >
{
	...
	public static < D extends RealType< D > & NativeType< D >, T extends AbstractVolatileNativeRealType< D, T > > RawSourceState< D, T > simpleSourceFromSingleRAI(
			...
			final String name )
	{
		...
		if ( !Views.isZeroMin( data ) ) { return simpleSourceFromSingleRAI( Views.zeroMin( data ), resolution, offset, min, max, name ); }
		...
		return new RawSourceState<>( dataSource, new ARGBColorConverter.InvertingImp0<>( min, max ), new CompositeCopy<>(), name );
