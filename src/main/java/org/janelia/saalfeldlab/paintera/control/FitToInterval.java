		...
		public void onChanged( final Change< ? extends Source< ? > > change )
		{
			while ( change.next() )
				if ( change.wasAdded() && change.getList().size() == 1 )
				{
					...
					final double[] min = Arrays.stream( Intervals.minAsLongArray( addedSource.getSource( 0, 0 ) ) ).asDoubleStream().toArray();
					final double[] max = Arrays.stream( Intervals.maxAsLongArray( addedSource.getSource( 0, 0 ) ) ).asDoubleStream().toArray();
					final AffineTransform3D tf = new AffineTransform3D();
