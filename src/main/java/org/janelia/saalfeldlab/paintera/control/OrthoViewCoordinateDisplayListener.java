...
public class OrthoViewCoordinateDisplayListener
{
	...
	public Consumer< ViewerPanelFX > onEnter()
	{
		return t -> {
			...
			{
				final CoordinateDisplayListener coordinateListener = new CoordinateDisplayListener( t, submitViewerCoordinate, submitWorldCoordinate );
				listeners.put( t, EventFX.MOUSE_MOVED( "coordinate update", e -> coordinateListener.update( e.getX(), e.getY() ), e -> true ) );
			}
