...
public class N5DataFromMetadata {
  ...
  ImagesWithTransform<T, V>[] openRawMultiscale(
		  ...
		  final PainteraMultiscaleGroup metadata,
		  final SharedQueue queue,
		  final int priority) throws IOException {

	final String[] scaleDatasets = metadata.sortedScaleDatasets();

	LOG.debug("Opening directories {} as multi-scale in {}: ", Arrays.toString(scaleDatasets), metadata.getPath());

	final double[] initialDonwsamplingFactors = metadata.getDownsamplingFactors(0);
	AffineTransform3D transform = metadata.getTransform();
	...
	for (int scale = 0; scale < scaleDatasets.length; ++scale) {
	  ...
	  futures.add(es.submit(ThrowingSupplier.unchecked(() -> {
		...
		final String scaleDataset = scaleDatasets[fScale];
		imagesWithInvalidate[fScale] = N5Data.openRaw(reader, scaleDataset, transform.copy(), queue, priority);
