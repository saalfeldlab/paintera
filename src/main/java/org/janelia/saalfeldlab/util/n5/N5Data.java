...
public class N5Data {
    ...
    ImagesWithTransform<T, V> openRaw(
        ...
        try {
            ...
            final CachedCellImg<T, ?> slice;
            if (raw.numDimensions() != 3) {
                RandomAccessible<T> extendedRaw = (RandomAccessible<T>)Views.extendZero((RandomAccessibleInterval)raw);
                final int[] cellSize = raw.getCellGrid().getCellDimensions();
                for (int d = 3; d < cellSize.length - 1; ++d) {
                    cellSize[d] = 1;
                }
                final long[] cellSizeLong = Arrays.stream(cellSize).mapToLong(it -> (long) it).toArray();
                var options = new DiskCachedCellImgOptions().cellDimensions(cellSize[0], cellSize[1], cellSize[2]);
                var dims3d = new long[]{raw.dimension(0), raw.dimension(1), raw.dimension(2)};
                slice = new DiskCachedCellImgFactory<T>(raw.getType(), options).create(dims3d, cell -> {
                    final long[] cellMin = raw.dimensionsAsLongArray();
                    for (int d = 0; d < cellMin.length; ++d) {
                        if (d < 3) cellMin[d] = cell.min(d);
                        else cellMin[d] = 0;
                    }
                    final long[] cellMinWithSize = LongStream.concat(
                            Arrays.stream(cellMin),
                            Arrays.stream(cellSizeLong)
                    ).toArray();
                    final FinalInterval cellInterval = Intervals.createMinSize(cellMinWithSize);
                    final Cursor<T> sliceCursor = Views.interval(extendedRaw, cellInterval).cursor();
            ...
            return new ImagesWithTransform<>(slice, vraw.getRai(), transform, slice.getCache(), vraw.getInvalidate());
