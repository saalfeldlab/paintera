 ...
 * {@link RandomAccess} has moved.  If the {@link RandomAccess} did not move,
 * the bounding box covers the complete range from
 * (2<sup>-63</sup>)<sup>n</sup> to (2<sup>63</sup>)<sup>n</sup> but with min
 * pointing at max and max pointing at min.  I.e. the initialization of the
 * {@link RandomAccess} at 0<sup>n</sup> is not considered part of the access
 * box.
 *
 * This {@link RandomAccessible} has a singleton {@link RandomAccess} (itself)
 * such that updates of the access box do not have to be synchronized.  If
 * using in multithreaded contexts, make many instances of the
