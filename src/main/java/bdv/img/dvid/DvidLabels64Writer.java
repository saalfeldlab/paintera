...
public class DvidLabels64Writer {
	...
	/**
	 * @param apiUrl		Url to dvid api in the form of http://hostname:port/api
	 * @param uuid			Uuid of repository within the dvid server specified by apiUrl
	 * @param dataSet		Name of the data set within the repository specified by uuid. This data set must be of type labelblk
	 * This calls {@link DvidLabels64Writer#DvidLabels64ByteWriter(String, String, String, int)} with a default block size of 32.
	 **/
	 ...
	 * @param uuid			Uuid of repository within the dvid server specified by apiUrl
	 * @param dataSet		Name of the data set within the repository specified by uuid. This data set must be of type labelblk
	 * @param blockSize		Block size of the data set. Must suit block size stored in dvid server.
	 */
	...
	/**
	 * @param image					Image to be stored in dvid server.
	 * @param iterationAxis			Along which axis to iterate.
	 * @param steps					Step sizes along each axis.
	 * @param offset				Offset target position by offset.
	 * Write image into data set. Calls {@link DvidLabels64Writer#writeImage(RandomAccessibleInterval, int, int[], int[])}
	 ...
	 * @param steps					Step sizes along each axis.
	 * @param offset				Offset target position by offset.
	 * Write image into data set. Calls {@link DvidLabels64Writer#writeImage(RandomAccessibleInterval, int, int[], int[], RealType)}
	 ...
	 * @param steps					Step sizes along each axis.
	 * @param offset				Offset target position by offset.
	 * @param borderExtension		Extend border with this value.
	 * Write image into data set. The image will be divided into blocks as
	 * defined by steps. The target coordinates will be the image coordinates
	 * shifted by offset.
	 */
	public < T extends RealType< T > > void writeImage(
			RandomAccessibleInterval< T > image,
			...
			T borderExtension )
	{
		...
		// Create BlockedInterval that allows for flat iteration and intuitive hyperslicing.
		// Go along iterationAxis and hyperslice, then iterate over each hyperslice.
	...
	/**
	 * @param input				Block of data to be written to dvid data set.
	 * @param dims				Interpretation of the dimensionality of the block, e.g. [0,1,2] corresponds to "xyz".
	 * @param offset			Position of the "upper left" corner of the block within the dvid coordinate system.
	 * @throws IOException
	 * Write block into dvid data set at position specified by offset using http POST request.
	 */
	...
	{
		// Offset and block dimensions must be integer multiples of this.blockSize.
		// For now add the asserts, maybe make this method private/protected and have
		...
		toByteArray.rewind(); // unnecessary because toArray() returns underlying array?
	...
	/**
	 * @param input			Integer array corresponding to be adapted (will be modified).
	 * @param blockSize		Block size.
	 * @return Modified input.
	 * When sending POST request to dvid, all block sizes and offsets need to be integral multiples of blockSize.
	 * This is a convenience function to modify integer arrays accordingly.
	 */
	...
	/**
	 * @param input			Long array corresponding to be adapted (will be modified).
	 * @param blockSize		Block size.
	 * @return Modified input.
	 * When sending POST request to dvid, all block sizes and offsets need to be integral multiples of blockSize.
	...
	/**
	 * This is for checking functionality. Adjust apiUrl, uuid and dataSet according to your needs. 
	 * @param args
	 * @throws IOException
	 */
	public static void main(String[] args) throws IOException {
		
		// this is for testing purposes, modify apiUrl, uuid and dataSet according to your needs
		// if values received from server differ from input, these values will be printed to stdout
		// otherwise no output
		...
		for ( FloatType r : ref ) r.set( rng.nextFloat() );
